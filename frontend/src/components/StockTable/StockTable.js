...
const StockTable = ({ stocks = [], resourcesMap = {}, onRefresh }) => {
  ...
  const handleSaveEdit = async (id) => {
    try {
      await api.patch(`/stocks/${id}/update_amount/`, { amount: parseFloat(editValue) });
      setEditingId(null);
    ...
    <section className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex flex-col h-[600px]">
      <div className="p-5 border-b border-slate-100 bg-slate-50/50 space-y-4">
        ...
        <div className="flex gap-3">
          <select value={filterCategory} onChange={(e) => setFilterCategory(e.target.value)} className="flex-1 p-2 rounded-xl border border-slate-200 text-xs">
            <option value="all">Усі категорії</option>
            {categories.filter(c => c !== 'all' && c).map(cat => <option key={cat} value={cat}>{cat}</option>)}
          </select>
          <select value={filterWarehouse} onChange={(e) => setFilterWarehouse(e.target.value)} className="flex-1 p-2 rounded-xl border border-slate-200 text-xs">
        ...
        <table className="w-full text-sm text-left">
          ...
          <tbody>
            {filteredStocks.map((stock) => {
              const resource = resourcesMap[stock.resource] || { name: '...', category_name: '...', unit_name: '?' };
                      ...
                      {isEditing ? (
                        <div className="flex items-center gap-1 animate-in fade-in zoom-in duration-200">
                          <input
                            ...
                            className="w-20 p-1 border-2 border-blue-500 rounded-lg text-right font-mono text-sm outline-none"
                          />фвв
                          <button onClick={() => handleSaveEdit(stock.id)} className="p-1 bg-green-500 text-white rounded-md">
                            <Check size={14} />
                          </button>
                        </div>
                      ) : (
                        ...
                        <div className="relative flex items-center justify-end w-full h-full">
                          ...
                          <button
                            onClick={() => { setEditingId(stock.id); setEditValue(stock.amount); }}
