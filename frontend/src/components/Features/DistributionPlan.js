  ...
  const exportPlanToExcel = () => {
    ...
    const dataToExport = plan.items.map(item => {
      ...
      return {
        ...
        "Всього було на складі": item.total_available_at_source || 'Н/Д',
    ...
    <div className="animate-fade-in-up bg-slate-50 rounded-3xl border-2 border-slate-200 p-6 md:p-8 mb-10 text-left space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-200 pb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-500 text-xs font-bold uppercase tracking-widest">Алгоритм лексикографічного розподілу</p>
          </div>
        </div>

        <div className="flex items-center gap-3 w-full md:w-auto">
          <button onClick={exportPlanToExcel} className="flex items-center justify-center gap-2 px-5 py-2.5 bg-white border-2 border-slate-200 rounded-xl font-black text-[11px] uppercase text-slate-600 hover:bg-slate-100 hover:border-slate-300 transition-all shadow-sm active:scale-95 flex-1 md:flex-none">
            <Download size={16} className="text-blue-600" /> Експорт (.xlsx)
          </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {plan.items.map((item) => {
          ...
          const purposeKey = item.purpose_code || item.purpose;
          const purposeData = PURPOSE_MAP[purposeKey] || { label: item.purpose_name || 'Інше', icon: <HelpCircle size={14} />, color: 'bg-slate-100 text-slate-800' };

          return (
            <div key={item.id} className="bg-white p-5 rounded-2xl border border-slate-200 hover:shadow-md transition-all flex flex-col justify-between group relative overflow-hidden animate-in fade-in zoom-in duration-300 shadow-sm">
